...
#import "ReverseMaskCipher.hpp"
#import "PrintExamplesHelper.hpp"
#import "CipherGauntlet.hpp"
#include <memory>

@interface Battery_ReverseMaskCipher : XCTestCase

@end

@implementation Battery_ReverseMaskCipher

- (void)test_gauntlet_ReverseMaskCipher {
    ...
    for (auto &aMask: cMasks) {
        ReverseMaskCipher aCipher(static_cast<std::uint8_t>(aMask));
        
        if (!aGauntlet.RunCipher(&aCipher,
                                 16,
                                 "ReverseMaskCipher")) {
            XCTFail("[ReverseMaskCipher] test_gauntlet_ReverseMaskCipher failed.");
            return;
        }
...
- (void)test_fixed_16_A_ReverseMaskCipher {
    ...
    ReverseMaskCipher aCrypt(aMaskByte);
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_A_ReverseMaskCipher failed after seal.");
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_A_ReverseMaskCipher failed after unseal.");
...
- (void)test_fixed_16_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_C_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_D_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_E_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_G_ReverseMaskCipher {
    ...
    // After Cipher:
    // bit0 10101010  10101010
    // bit1 01010101  10010101
    // bit2 10010110  10100101
    // bit3 10100101  10101001
    // bit4 01100110  10011001
    // bit5 01100101  10011001
    // bit6 01011010  10010110
    // bit7 01101001  10011010
    //
    // Avalanche:
    // bit0 ........  ........
    // bit1 XXXXXX..  ..XXXXXX
    ...
    std::uint8_t aMaskByte = 0x51;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[16] = {
        0xB0, 0x4F, 0x9D, 0x62, 0x83, 0x7C, 0xAA, 0x55, 0xFF, 0x00, 0xB0, 0x4F, 0x9D, 0x62, 0x83, 0x7C
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_G_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_G_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_G_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_G_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_G_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_G_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_H_ReverseMaskCipher {
    ...
    // After Cipher:
    // bit0 10101010  10101010
    // bit1 10011010  10100110
    // bit2 01011010  01010110
    // bit3 01101010  01011010
    // bit4 01011001  10010110
    // bit5 01011001  10010110
    // bit6 10101001  01101010
    ...
    const std::uint8_t aExpected[16] = {
        0xC3, 0x3C, 0x92, 0x6D, 0xFF, 0x00, 0xF0, 0x0F, 0xCC, 0x33, 0xC3, 0x3C, 0x92, 0x6D, 0xFF, 0x00
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_H_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_H_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_H_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_H_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_H_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_H_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_I_ReverseMaskCipher {
    ...
    // bit4 00101111  00001011
    // bit5 00001111  01000011
    // bit6 00011001  11000110
    // bit7 00001011  11000010
    //
    // Avalanche:
    // bit0 XXXXXXXX  XXXXXXXX
    // bit1 ........  ........
    ...
    const std::uint8_t aExpected[16] = {
        0x70, 0xB0, 0x28, 0x82, 0x4F, 0xBC, 0x7D, 0x8F, 0x43, 0xD7, 0x70, 0xB0, 0x28, 0x82, 0x4F, 0xBC
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_I_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_I_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_I_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_I_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_I_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_I_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_J_ReverseMaskCipher {
    ...
    std::uint8_t aMaskByte = 0xD5;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[16] = {
        0x5D, 0xC4, 0x5D, 0xC4, 0x5D, 0xC4, 0x5D, 0xC4, 0x5D, 0xC4, 0x5D, 0xC4, 0x5D, 0xC4, 0x5D, 0xC4
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_J_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_J_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_J_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_J_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_J_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_J_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_K_ReverseMaskCipher {
    ...
    std::uint8_t aMaskByte = 0xB3;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[16] = {
        0x73, 0x8C, 0x73, 0x8C, 0x73, 0x8C, 0x73, 0x8C, 0x4F, 0xB0, 0x4F, 0xB0, 0x4F, 0xB0, 0x4F, 0xB0
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_K_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_K_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_K_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_K_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_K_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_K_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_16_L_ReverseMaskCipher {
    ...
    //
    // After Cipher:
    // bit0 00010111  10100001
    // bit1 10000101  01010101
    // bit2 00010101  01010101
    // bit3 10101000  01010101
    // bit4 01010000  10101000
    // bit5 10101000  00001010
    // bit6 01111111  11101000
    // bit7 10101010  11111110
    //
    // Avalanche:
    // bit0 X..X..X.  .X..X..X
    // bit1 ........  ........
    // bit2 X.XXXXXX  XXXXXX.X
    // bit3 ......X.  .X......
    // bit4 ........  ........
    // bit5 XXXXX...  ...XXXXX
    // bit6 ........  ........
    // bit7 XX.X.X.X  X.X.X.XX
    // Avalanche: 32.81%
    ...
    const std::uint8_t aExpected[16] = {
        0x55, 0x0A, 0x17, 0xAA, 0x17, 0xE2, 0x83, 0xE2, 0x8B, 0x73, 0x8B, 0x71, 0x0F, 0x71, 0x05, 0xF0
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_16_L_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_L_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_L_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_16_L_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_L_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_16_L_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_C_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_D_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_E_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_G_ReverseMaskCipher {
    ...
    // bit1 10101001  10101010  01101010  10011010
    // bit2 01010110  10010101  10100101  01101001
    // bit3 10010101  10100101  01101001  01011010
    // bit4 10010110  01100101  10011001  01100110
    // bit5 01100101  10011001  01100110  01011001
    // bit6 01011010  10010110  10100101  10101001
    // bit7 01011001  10010110  01100101  10011001
    //
    ...
    std::uint8_t aMaskByte = 0xA8;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[32] = {
        0x58, 0xA7, 0x44, 0xBB, 0x43, 0xBC, 0x2A, 0xD5, 0x77, 0x88, 0x58, 0xA7, 0x44, 0xBB, 0x43, 0xBC,
        0x2A, 0xD5, 0x77, 0x88, 0x58, 0xA7, 0x44, 0xBB, 0x43, 0xBC, 0x2A, 0xD5, 0x77, 0x88, 0x58, 0xA7
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_G_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_G_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_G_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_G_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_G_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_G_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_H_ReverseMaskCipher {
    ...
    // After Cipher:
    // bit0 10101010  10101010  10101010  10101010
    // bit1 10101010  01101010  10011010  10100110
    // bit2 01011010  01010110  10010101  10100101
    // bit3 01101010  01011010  10010110  10100101
    // bit4 01011001  10010110  01100101  10011001
    // bit5 10011001  01100110  01011001  10010110
    // bit6 01010110  10010101  10100101  01101001
    // bit7 01100110  10011001  10100110  01101001
    //
    // Avalanche:
    // bit0 XXXXXXXX  XXXXXXXX  XXXXXXXX  XXXXXXXX
    // bit1 XX..XXXX  ..XX..XX  XX..XX..  XXXX..XX
    // bit2 ........  ........  ........  ........
    // bit3 ........  ........  ........  ........
    // bit4 ........  ........  ........  ........
    ...
    const std::uint8_t aExpected[32] = {
        0xC4, 0x3B, 0xD1, 0x2E, 0xFC, 0x03, 0xF3, 0x0C, 0x8B, 0x74, 0xC4, 0x3B, 0xD1, 0x2E, 0xFC, 0x03,
        0xF3, 0x0C, 0x8B, 0x74, 0xC4, 0x3B, 0xD1, 0x2E, 0xFC, 0x03, 0xF3, 0x0C, 0x8B, 0x74, 0xC4, 0x3B
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_H_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_H_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_H_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_H_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_H_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_H_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_I_ReverseMaskCipher {
    ...
    // bit1 10001010  11100010  10111000  10101110
    // bit2 11100110  00111001  10001110  01100011
    // bit3 11100110  00111001  10001110  01100011
    // bit4 00101111  00001011  11000010  11110000
    // bit5 00101111  00001011  11000010  11110000
    // bit6 10011100  01100111  00011001  11000110
    // bit7 10111100  00101111  00001011  11000010
    //
    // Avalanche:
    // bit0 XXXXXXXX  XXXXXXXX  XXXXXXXX  XXXXXXXX
    // bit1 ........  ........  ........  ........
    ...
    std::uint8_t aMaskByte = 0x97;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[32] = {
        0x73, 0xB0, 0x3D, 0x83, 0x4F, 0xBF, 0x7C, 0x8C, 0x40, 0xC2, 0x73, 0xB0, 0x3D, 0x83, 0x4F, 0xBF,
        0x7C, 0x8C, 0x40, 0xC2, 0x73, 0xB0, 0x3D, 0x83, 0x4F, 0xBF, 0x7C, 0x8C, 0x40, 0xC2, 0x73, 0xB0
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_I_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_I_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_I_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_I_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_I_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_I_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_J_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_J_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_J_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_J_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_J_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_J_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_J_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_K_ReverseMaskCipher {
    ...
    // bit5 01010101  10101010  01010101  01010101
    // bit6 01010101  01010101  10101010  10101010
    // bit7 10101010  01010101  10101010  10101010
    //
    // Avalanche:
    // bit0 ........  ........  ........  ........
    // bit1 ........  ........  ........  ........
    ...
    std::uint8_t aMaskByte = 0x0B;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[32] = {
        0xF1, 0x0E, 0xF1, 0x0E, 0xF1, 0x0E, 0xF1, 0x0E, 0xCC, 0x33, 0xCC, 0x33, 0xCC, 0x33, 0xCC, 0x33,
        0xC3, 0x3C, 0xC3, 0x3C, 0xC3, 0x3C, 0xC3, 0x3C, 0xAB, 0x54, 0xAB, 0x54, 0xAB, 0x54, 0xAB, 0x54
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_K_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_K_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_K_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_K_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_K_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_K_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_32_L_ReverseMaskCipher {
    ...
    //
    // After Cipher:
    // bit0 10100001  01111010  00010111  10100001
    // bit1 00000001  01111010  10101010  10100001
    // bit2 11111110  10101010  00010101  01010101
    // bit3 10101010  00010101  01010101  01111010
    // bit4 11110101  01010000  00010101  00001010
    // bit5 01010101  01010000  10101000  00001010
    // bit6 01111111  11101000  00000001  01010101
    // bit7 01111111  01010101  00000001  01010000
    //
    // Avalanche:
    // bit0 ..X..X..  X..X..X.  .X..X..X  ..X..X..
    // bit1 X....X..  ..X.XXXX  XXXX.X..  ..X....X
    // bit2 .X.X.X..  ......X.  .X......  ..X.X.X.
    // bit3 ........  ........  ........  ........
    // bit4 X.X..X.X  XXXXX...  ...XXXXX  X.X..X.X
    ...
    std::uint8_t aMaskByte = 0xEC;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[32] = {
        0xB8, 0x2F, 0xBB, 0x2F, 0x33, 0x2F, 0x33, 0xCF, 0x22, 0xCF, 0xE2, 0xDD, 0xE2, 0x11, 0xE0, 0x11,
        0x44, 0x10, 0x44, 0xB8, 0x44, 0xB8, 0xC0, 0xBB, 0xC0, 0x33, 0xD0, 0x33, 0x1C, 0x22, 0x1C, 0xE2
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_32_L_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_L_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_L_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_32_L_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_L_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_32_L_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_48_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_48_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_48_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_48_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_48_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_48_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_48_C_ReverseMaskCipher {
    ...
    std::uint8_t aMaskByte = 0xAB;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[48] = {
        0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4,
        0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4,
        0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4, 0x5B, 0xA4
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_48_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_48_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_48_D_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_48_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_48_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_48_E_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[48] = {
        0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5,
        0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5,
        0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5, 0x1A, 0xE5
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_48_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_48_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_48_F_ReverseMaskCipher {
    ...
    std::uint8_t aMaskByte = 0xD6;

    ReverseMaskCipher aCrypt(aMaskByte);

    const std::uint8_t aExpected[48] = {
        0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83,
        0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83,
        0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83, 0x7C, 0x83
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_48_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_48_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_48_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_64_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_64_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_64_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_64_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_64_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_64_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_64_C_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_64_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_64_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_64_D_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[64] = {
        0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8,
        0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8,
        0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8,
        0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8, 0x57, 0xA8
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_64_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_64_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_64_E_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[64] = {
        0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91,
        0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91,
        0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91,
        0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91, 0x6E, 0x91
    };
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_64_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_64_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_64_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_64_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_64_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_64_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_80_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_80_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_80_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_80_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_80_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_80_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_80_C_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_80_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_80_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_80_D_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[80] = {
        0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45,
        0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45,
        0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45,
        0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45,
        0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45, 0xBA, 0x45
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_80_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_80_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_80_E_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_80_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_80_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_80_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_80_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_80_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_80_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_96_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_96_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_96_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_96_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_96_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_96_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_96_C_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[96] = {
        0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB,
        0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB,
        0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB,
        0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB,
        0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB, 0x34, 0xCB,
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_96_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_96_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_96_D_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[96] = {
        0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51,
        0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51,
        0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51,
        0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51,
        0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51, 0xAE, 0x51,
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_96_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_96_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_96_E_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_96_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_96_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_96_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_96_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_96_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_96_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_112_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_112_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_112_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_112_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_112_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_112_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_112_C_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[112] = {
        0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A,
        0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A,
        0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A,
        0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A,
        0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A, 0x75, 0x8A,
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_112_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_112_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_112_D_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_112_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_112_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_112_E_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_112_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_112_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_112_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_112_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_112_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_112_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_128_A_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_128_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_A_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_A_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_128_A_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_A_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_A_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_128_B_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_128_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_B_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_B_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_128_B_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_B_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_B_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_128_C_ReverseMaskCipher {
    ...
    const std::uint8_t aExpected[128] = {
        0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4,
        0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4,
        0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4,
        0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4,
        0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4, 0x3B, 0xC4,
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_128_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_C_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_C_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_128_C_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_C_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_C_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_128_D_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_128_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_D_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_D_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_128_D_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_D_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_D_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_128_E_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_128_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_E_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_E_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_128_E_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_E_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_E_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
...
- (void)test_fixed_128_F_ReverseMaskCipher {
    ...
    if (!CipherGauntlet::SealDataExpectingSuccess(aOriginal,
                                                    ...
                                                    sizeof(aOriginal),
                                                    &aCrypt,
                                                    "test_fixed_128_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_F_ReverseMaskCipher failed while sealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aExpected, aEncrypted, sizeof(aExpected))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_F_ReverseMaskCipher failed after seal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aExpected, sizeof(aExpected)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aEncrypted, sizeof(aEncrypted)).c_str());
        return;
    }
    ...
    if (!CipherGauntlet::UnsealDataExpectingSuccess(aEncrypted,
                                                      ...
                                                      &aCrypt,
                                                      "test_fixed_128_F_ReverseMaskCipher")) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_F_ReverseMaskCipher failed while unsealing.");
        return;
    }

    if (!CipherGauntlet::Matches(aOriginal, aUnencrypted, sizeof(aOriginal))) {
        XCTFail("[ReverseMaskCipher] test_fixed_128_F_ReverseMaskCipher failed after unseal.");
        printf("Expected: %s\n", PrintExamplesHelper::HexArray(aOriginal, sizeof(aOriginal)).c_str());
        printf("Actual: %s\n", PrintExamplesHelper::HexArray(aUnencrypted, sizeof(aUnencrypted)).c_str());
        return;
    }
