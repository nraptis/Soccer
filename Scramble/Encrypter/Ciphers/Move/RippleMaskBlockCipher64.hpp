#ifndef JELLY_RIPPLE_MASK_BLOCK_CIPHER_64_HPP_
#define JELLY_RIPPLE_MASK_BLOCK_CIPHER_64_HPP_
...
class RippleMaskBlockCipher64 final : public Cipher {
 public:
  explicit RippleMaskBlockCipher64(std::uint8_t pMask)
      : Cipher(CipherType::kRippleMaskBlockCipher64),
        mMask(pMask) {}
  ...
  void BlendBlocksSimd(std::uint8_t *pDestination,
    ...
    for (std::size_t aOffset = 0u; aOffset < kBlockSize; aOffset += 32u) {
      const __m256i aBase = _mm256_loadu_si256(
          reinterpret_cast<const __m256i *>(pBaseBlock + aOffset));
      const __m256i aMoved = _mm256_loadu_si256(
          reinterpret_cast<const __m256i *>(pMovedBlock + aOffset));
...
#endif  // JELLY_RIPPLE_MASK_BLOCK_CIPHER_64_HPP_
