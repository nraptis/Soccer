#ifndef JELLY_CASCADE_XOR_CIPHER_HPP_
#define JELLY_CASCADE_XOR_CIPHER_HPP_
...
// Input shape: pLength must be 0 or a multiple of 16; mask buffer is pLength bytes.

class CascadeXORCipher final : public Crypt {
public:
    explicit CascadeXORCipher(const std::uint8_t *pMask) : mMask(pMask) {}
...
private:
    ...
    bool Apply(const std::uint8_t *pSource,
        ...
        for (std::size_t aChunk = 0; aChunk < aChunkCount; ++aChunk) {
            ...
            ++aMaskChunkIndex;
            if (aMaskChunkIndex == aChunkCount) {
                aMaskChunkIndex = 0u;
            }
        }
        return true;
...
#endif  // JELLY_CASCADE_XOR_CIPHER_HPP_
